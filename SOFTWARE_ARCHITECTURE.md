...
- `src/features/motorcycles/new-motorcycle/` owns the isolated production dialog for registering a Motorcycle to an authoritative Customer context. It loads active make, color, and plate-code references through the typed API, performs only required-field and whole-year shape checks, preserves textual identity input for the Rust domain, and returns the existing joined Motorcycle projection to its caller. It is intentionally not mounted in the application shell yet.
